function[estimated_distance,theta_head,phi_head,normal_surface_vector,yaw_value]= measurement_front_direction(obstacul,i,j,k,l,m,theta_drone,phi_drone,dir_front,dir_up,dir_rside,direction_to_measure,pos_drone_x,pos_drone_y,pos_drone_z)
% gol=1
yaw_value=0;
theta_head=100;
phi_head=100;
...
for u=1:k
    ...
    t_v=(center_x-side_x/2-pos_drone_x)/direction_to_measure(1);
    
    if (isreal(t_v)&& (length(t_v)>0)&& (t_v>=0))
        y_val=pos_drone_y+t_v*direction_to_measure(2);
        z_val=pos_drone_z+t_v*direction_to_measure(3);
        
        
        vect_1=[-1;0;0];
        if (pos_drone_x<center_x-side_x/2) 
            vect_2=-[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];%%%estoy afuera
        else
            vect_2=[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];
        end
        angle_bet = atan2(norm(cross(vect_1,vect_2)),dot(vect_1,vect_2));
    ...
    %%%%plano y= a algo 11111111111111
    t_v=(center_y-side_y/2-pos_drone_y)/direction_to_measure(2);
    
    
    if (isreal(t_v)&& (length(t_v)>0)&& (t_v>=0))
        x_val=pos_drone_x+t_v*direction_to_measure(1);
        z_val=pos_drone_z+t_v*direction_to_measure(3);
        
        
        vect_1=[0;-1;0];
        if (pos_drone_y<center_y-side_y/2) vect_2=-[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];%%%estoy afuera
        else vect_2=[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];
        end
    ...
    %%%%plano y= a algo 2222222222222222222
    t_v=(center_y+side_y/2-pos_drone_y)/direction_to_measure(2);
    ...
    %%%%%plano z= a algo
    t_v=(center_z-side_z/2-pos_drone_z)/direction_to_measure(3);
    ...
    if (isreal(t_v)&& (length(t_v)>0)&& (t_v>=0))
        ...
        vect_1=[0;0;-1];
        if (pos_drone_z<center_z-side_z/2) vect_2=-[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];%%%estoy afuera
        else vect_2=[direction_to_measure(1);direction_to_measure(2);direction_to_measure(3)];
        end
        angle_bet = atan2(norm(cross(vect_1,vect_2)),dot(vect_1,vect_2));
...
%     pos_drone_x
    ...
    %     alpha=pos_drone_x-center_x;
    %     beta=pos_drone_y-center_y;
    %     a=dir_front(1)^2+dir_front(2)^2;
    %     b=(2*(alpha)*dir_front(1))+(2*(beta)*dir_front(2));
    %     c=alpha^2+beta^2-radius^2;
