...
function [dist_front,dist_bottom,dist_left,dist_right,dist_back,theta_head,phi_head,normal_surface_vector,yaw_value] = distance_estimation_v_2(obstacul,constants,droneState,pos_drone_x,pos_drone_y,pos_drone_z,psi,theta,phi)
...
i=length (obstacul.cilindre);
j=length (obstacul.sphere);
k=length (obstacul.cube);
l=length (obstacul.plan_incl);
m=length (obstacul.cilindre_y_z);
n=length (obstacul.paraboloid_y_z);
...
if (dist_front < dist_min_detected)
    dist_front=dist_min_detected;
end
if (dist_front > dist_max_detected)
    dist_front=dist_max_detected;
end     
...
if (dist_back < dist_min_detected)
    dist_back=dist_min_detected;
end
if (dist_back > dist_max_detected)
    dist_back=dist_max_detected;
end     
...
if (dist_left < dist_min_detected)
    dist_left=dist_min_detected;
end
if (dist_left > dist_max_detected)
    dist_left=dist_max_detected;
end
...
if (dist_right < dist_min_detected)
    dist_right=dist_min_detected;
end
if (dist_right > dist_max_detected)
    dist_right=dist_max_detected;
end
...
if (dist_bottom < dist_min_detected)
    dist_bottom=dist_min_detected;
end
if (dist_bottom > dist_max_detected)
    dist_bottom=dist_max_detected;
end
end